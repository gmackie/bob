...
const corsOptions = {
  origin: function (
    ...
    if (origin && origin.endsWith(".ngrok.app")) {
      callback(null, true);
      return;
    }
...
app.use(
  "/api/instances",
  createInstanceRoutes(agentService, terminalService, gitService),
);
...
app.get("/api/system-status", async (req, res) => {
  try {
    ...
    const totalWorktrees = repositories.reduce((count: number, repo: any) => {
      const actualWorktrees = repo.worktrees.filter(
        (worktree: any) => !worktree.isMainWorktree,
      );
