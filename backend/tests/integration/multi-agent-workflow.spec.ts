...
describeIntegration("Multi-Agent Workflow Integration Tests", () => {
  ...
  describe("Agent Switching Scenarios", () => {
    it("should switch from one agent to another", async () => {
      ...
      // Start with Claude
      const claudeInstance = await agentService.startInstance(
        worktree.id,
        "claude",
      );
      ...
      // Start with Gemini
      const geminiInstance = await agentService.startInstance(
        worktree.id,
        "gemini",
      );
    ...
    it("should handle multiple agents for same repository", async () => {
      ...
      // Start instances
      const instance1 = await agentService.startInstance(
        worktree1.id,
        "claude",
      );
