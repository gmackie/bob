      ...
      try {
        ...
        if (process.platform === "linux") {
          try {
            ...
            expect(stat.slice(stat.lastIndexOf(")") + 2).split(" ")[0]).toMatch(
              /^[ZX]$/,
            );
          } catch (error) {
            if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
          }
        } else expect(() => process.kill(grandchildPid, 0)).toThrow();
      } finally {
        try {
          process.kill(-child.pid!, "SIGKILL");
        } catch {}
...
describe.skipIf(process.platform === "win32")(
  ...
  () => {
    it("waits for disappearance after an EPERM probe instead of claiming shutdown", async () => {
      ...
      try {
        ...
        expect(kill).toHaveBeenCalledWith(
          process.platform === "win32" ? 432109 : -432109,
          0,
        );
    ...
    it("still rejects when permission prevents the actual termination signal", async () => {
      ...
      try {
        await expect(terminateProcessTree(432109)).rejects.toMatchObject({
          code: "EPERM",
        });
      } finally {
        kill.mockRestore();
