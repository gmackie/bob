...
import { makeRpcHandler, BobRpcGroup } from "@bob/api/rpc-server";
import { makeRestBridge } from "@bob/api/rest-bridge";
...
export function getRpcMount(): RpcMount {
  ...
  const restBridge = makeRestBridge(BobRpcGroup, rpcHandler);
