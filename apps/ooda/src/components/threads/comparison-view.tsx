...
export function ComparisonView({
  ...
  return (
    ...
    >
      {/* Amber bar header */}
      <div className="flex items-center justify-between border-b border-primary/40 bg-[#2E2A1A] px-4 py-2">
        <span className="font-mono text-xs font-medium text-primary">
          Comparison Mode
        </span>
        <button
          onClick={onClose}
          className="rounded-[3px] border border-primary/30 px-2 py-0.5 font-mono text-xs text-primary hover:bg-primary/10"
        >
          Close
        </button>
      </div>
              ...
              <div className="flex items-center gap-2 border-b border-border bg-card px-4 py-2">
                ...
                <span
                  className={`rounded-[3px] px-1.5 py-0.5 text-xs ${
                    STATUS_STYLES[stream.status] ?? "bg-card text-muted-foreground"
                  }`}
                >
                  {stream.status}
                </span>
              ...
              {stream.status === "completed" && stream.output && (
                <div className="border-t border-border bg-card px-4 py-2">
                  <button
                    ...
                    className="rounded-[3px] border border-primary/30 bg-primary/10 px-3 py-1 font-mono text-xs text-primary hover:bg-primary/20 disabled:opacity-50"
                  >
