...
type InboxItem = {
  ...
  // Non-null since inboxByThread now uses an INNER JOIN on
  // standing_interest (see research.ts comment). Keeping this string-only
  // so the cache-update branch can round-trip without null-widening.
  standingInterestLabel: string;
};

// Default KB slug when the user promotes without picking one. The
// approval step for drafts (Task 4.4) happens in the vault UI later —
// landing in `unassigned` keeps drafts visible there without forcing the
// user to pre-categorize mid-triage.
  ...
  const promoteMutation = useMutation(
    trpc.research.kbPromoteRequest.mutationOptions({
      onMutate: async (vars: { sourceIds: number[] }) => {
        // kbPromoteRequest uses sourceIds, not the inbox row id. Find the
        // matching inbox row in the cache and strip it optimistically.
        await queryClient.cancelQueries({ queryKey });
        const previous = queryClient.getQueryData<{ items: InboxItem[] }>(queryKey);
        if (previous) {
          queryClient.setQueryData<{ items: InboxItem[] }>(queryKey, {
      ...
      {items.map((item) => (
        <InboxRow
          ...
          onPromote={(noteMd) => {
            ...
            // Always mark the row promoted server-side too so it doesn't
            // re-appear in the pending bucket after the draft is written.
