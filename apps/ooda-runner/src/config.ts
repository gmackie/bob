...
export function loadConfig(): RunnerConfig {
  return RunnerConfigSchema.parse({
    ...
    inferenceHost: process.env.OODA_INFERENCE_HOST,
    t3codeServerUrl:
      process.env.OODA_T3CODE_SERVER_URL ?? process.env.T3CODE_SERVER_URL,
    t3codeAuthToken:
      process.env.OODA_T3CODE_AUTH_TOKEN ?? process.env.T3CODE_AUTH_TOKEN,
    t3codeProjectId:
      process.env.OODA_T3CODE_PROJECT_ID ?? process.env.T3CODE_PROJECT_ID,
    t3codeModelInstanceId:
      process.env.OODA_T3CODE_MODEL_INSTANCE_ID ??
      process.env.T3CODE_MODEL_INSTANCE_ID,
