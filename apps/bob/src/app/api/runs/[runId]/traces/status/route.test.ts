...
beforeEach(() => {
  vi.resetAllMocks();
  vi.stubEnv("FORGEGRAPH_TRACE_API_TOKEN", "private-token");
});
describe("run trace storage status", () => {
  ...
  it("does not substitute a legacy integration credential for platform service auth", async () => {
    vi.stubEnv("FORGEGRAPH_TRACE_API_TOKEN", "");
    vi.stubEnv("FG_API_TOKEN", "legacy-integration-token");
    const run = { sessionId: "owned", artifacts: [] };
    mocks.get.mockResolvedValue(run);
    mocks.lookup.mockResolvedValue([]);
    await GET(new Request("https://bob.example"), {
      params: Promise.resolve({ runId: "run" }),
    });
    expect(mocks.lookup).toHaveBeenCalledWith(run, null);
  });
});
