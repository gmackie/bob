...
export default async function TaskWorkspacePage({
  ...
  const [detail, comments, taskRuns] = await Promise.all([
    caller("workItem.get").call({ id: workItemId }),
    caller("workItem.comment.list").call({ workItemId }),
    caller("workItem.taskRun.listByWorkItem").call({ workItemId }),
  ]);
  ...
  const [activeSession, workflowState] = activeSessionId
    ? await Promise.all([
        caller("agent.session.get").call({ id: activeSessionId }),
        caller("agent.session.getWorkflowState").call({
          sessionId: activeSessionId,
        }),
      ])
    : [null, null];
  const validationState = deriveTaskWorkspaceValidationState(
    detail.currentArtifacts.map((artifact) => ({
      ...artifact,
      url: artifact.url ?? "",
  ...
  const workItemHref = getWorkItemEntryHref(
    detail.workItem.id,
    "planning",
    selectedWorkspaceId,
  );
