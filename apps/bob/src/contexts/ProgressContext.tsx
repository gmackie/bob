...
type ProgressAction =
  | {
      ...
      payload: Omit<ProgressOperation, "id" | "startTime" | "status">;
    }
  | {
      type: "UPDATE_PROGRESS";
      payload: { id: string; progress: number; description?: string };
    }
  | {
...
interface ProgressContextValue {
  operations: ProgressOperation[];
  startOperation: (
    operation: Omit<ProgressOperation, "id" | "startTime" | "status">,
  ) => string;
...
export const ProgressProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  React.useEffect(() => {
    ...
    for (const operation of state.operations) {
      if (
        ...
        const delay = operation.status === "completed" ? 3000 : 8000;
  ...
  const updateProgress = useCallback(
    (id: string, progress: number, description?: string) => {
      dispatch({
        type: "UPDATE_PROGRESS",
        payload: { id, progress, description },
      });
    },
    [],
  );
