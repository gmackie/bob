...
describe("authorized run trace storage verification", () => {
  ...
  it("rejects redirects without following them or trusting their response body", async () => {
    const fetcher = vi.fn(async (_url, init) => {
      ...
      return Response.json(
        { results: [{ traceId, spanId, state: "stored", matchedSpanCount: 1 }] },
        { status: 307, headers: { location: "https://untrusted.example/collect" } },
      );
    });
    expect(await getRunTraceStatuses(run, config, fetcher)).toEqual([
