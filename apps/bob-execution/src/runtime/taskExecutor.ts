...
function buildPrRepairPrompt(
  ...
  lines.push(
    ...
    `This is NOT a "create a branch and open a PR" task. You are fixing an EXISTING pull request IN PLACE by adding a commit to its OWN branch (${input.headBranch}) and pushing that branch. If you have a GitHub/PR skill or habit that says to create a new branch and open a PR, IGNORE it entirely here.`,
    `Do NOT run \`git worktree add\`, \`git checkout -b <newname>\`, \`git switch -c\`, or \`git branch <newname>\`. Do NOT create /tmp worktrees. Do NOT open a PR. Your commit MUST land on \`${input.headBranch}\` and be pushed to origin, or it is worthless (an unpushed commit on a side branch is the #1 way repairs silently fail).`,
    "",
    "## What to do",
    `1. Get onto the PR's head branch in THIS worktree (do not make a new one):`,
    `   git fetch origin ${input.headBranch}`,
    `   git checkout ${input.headBranch} || git checkout -b ${input.headBranch} origin/${input.headBranch}`,
    `   Confirm with \`git branch --show-current\` that you are on ${input.headBranch} before editing.`,
    ...
    `   pnpm lint`,
    `   pnpm test`,
    `5. ONLY IF you made a real change that addresses the failure, commit it ON ${input.headBranch} (the branch you are already on — do NOT switch or create a branch) and push so the existing PR updates and CI re-runs:`,
    `   git add -A && git commit -m "fix: resolve merge blockers for #${input.number}"`,
    `   git push origin HEAD:${input.headBranch}`,
    `6. VERIFY the push landed (this is the whole point): after pushing, run`,
    `   git rev-parse HEAD  and  git ls-remote origin ${input.headBranch}`,
    `   and confirm they match. If your commit is not on the remote ${input.headBranch}, the repair FAILED — fix the push, do not stop until the remote head matches your commit.`,
    "",
    "## Hard rules",
    `- Commit ONLY on ${input.headBranch} (already checked out). NEVER create a new branch/worktree, NEVER \`git worktree add\`/\`checkout -b\`/\`switch -c\`. Do NOT open a new PR, do NOT close this one, do NOT touch ${baseBranch} or any other branch. An unpushed fix on a side branch is a silent failure — the most common way repairs are wasted.`,
