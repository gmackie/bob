...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bob-rotating-log-"));
  tempRoots.push(dir);
  return dir;
}
...
describe("RotatingFileSink", () => {
  it("rotates when writes exceed max bytes", () => {
    ...
    expect(fs.readFileSync(path.join(dir, "main.log"), "utf8")).toBe("abc");
    expect(fs.readFileSync(path.join(dir, "main.log.1"), "utf8")).toBe(
      "1234567890",
    );
  });
  ...
  it("prunes stale backups above maxFiles on startup", () => {
    const dir = makeTempDir();
    const logPath = path.join(dir, "main.log");
    fs.writeFileSync(path.join(dir, "main.log.1"), "first");
    fs.writeFileSync(path.join(dir, "main.log.4"), "stale");
    ...
    expect(fs.existsSync(path.join(dir, "main.log.4"))).toBe(false);
  });

  it("writeLine appends a newline when missing", () => {
    const dir = makeTempDir();
    const logPath = path.join(dir, "main.log");
    const sink = new RotatingFileSink({
      filePath: logPath,
