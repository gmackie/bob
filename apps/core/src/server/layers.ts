...
import { readDatabaseConfig } from "@gmacko/core/db/client";
...
import { createDatabaseManager } from "@gmacko/core/db/migrate";
import {
  ApiKeys,
  AuthMiddleware,
  initAuth,
...
import { layerRealtime, makeRealtimeChannelTag } from "@gmacko/core/realtime";

import { getServerEnv } from "./env.js";

// ---------------------------------------------------------------------------
// Singleton DB — survives module HMR in `next dev` and is reused across
// route invocations. Initialization includes the selected driver migrations.
// No connection is opened during module evaluation.
// ---------------------------------------------------------------------------

declare global {
  // eslint-disable-next-line no-var
  var __gmacko_database_manager:
    | ReturnType<typeof createDatabaseManager>
    | undefined;
}
const databaseManager = (globalThis.__gmacko_database_manager ??=
  createDatabaseManager(readDatabaseConfig(process.env)));

// ---------------------------------------------------------------------------
// Authentication and service layers are built after shared DB initialization.
// Failed initialization is retryable on the next request.
// ---------------------------------------------------------------------------

async function createServerLayers() {
  ...
  const AgentEventsChannel = makeRealtimeChannelTag<AgentEvent>(
  ...
  return { authInstance, runtimeLayer, authMiddlewareLayer };
}
let layers: ReturnType<typeof createServerLayers> | undefined;
export function getServerLayers() {
  layers ??= createServerLayers().catch((error) => {
    layers = undefined;
    throw error;
  });
  return layers;
...
// `@gmacko/auth`.
export { Sessions, ApiKeys, Tenancy, AuthMiddleware };
