...
export function useSessionSocket({
  ...
  const subscribedSessionsRef = useRef<Map<string, SubscribedSession>>(new Map());
  ...
  const getReconnectDelay = useCallback((attempt: number) => {
    const delay = Math.min(baseReconnectDelayMs * Math.pow(2, attempt), 30000);
    ...
    switch (msg.type) {
      ...
      case "event": {
        ...
        if (event.eventType === "state" && event.payload.status) {
          onStatusChange?.(event.sessionId, event.payload.status as SessionStatus);
        }
  ...
  }, [connect, maxReconnectAttempts, getReconnectDelay, clearReconnectTimers, updateConnectionState]);
