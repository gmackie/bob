...
export async function POST(request: NextRequest) {
  ...
  if (webhook.secret && !verifyGiteaSignature(payload, signature, webhook.secret)) {
    deliveryRecord.success = false;
      ...
      case "issues": {
        ...
        if ((action === "opened" || action === "closed" || action === "reopened") && issue.number) {
          ...
          const [linkedIssue] = await db
            ...
            .where(
              and(
                eq(issues.externalIssueProvider, "gitea"),
            ...
            if (syncDirection !== "outbound_only") {
              ...
              await db.insert(activities).values({
                ...
                changes: {
                  ...
                  from: priorStatus,
                  to: newStatus,
                  reason: `Gitea issue #${issue.number} ${action}`,
                },
              });
            }
          }
