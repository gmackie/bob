...
import { buildReport, saveReport, loadReport, loadReportOrFailure, sendReport, settledReport } from "./ci-report.mjs";
...
test("missing artifacts report failure even when native jobs claim success", async () => {
  const dir = await mkdtemp(join(tmpdir(), "bob-ci-missing-"));
  try {
    const path = join(dir, "report.json");
    const context = { ...env, CI_SOURCE_RESULT: "success", CI_RESEARCH_RESULT: "success" };
    const report = await loadReportOrFailure(path, context);
    assert.equal(report.status, "failure");
    assert.equal(report.sha, env.CI_CHECK_SHA);
    assert.equal(report.tests, undefined);
    assert.equal((await loadReportOrFailure(path, { ...context, CI_SOURCE_RESULT: "cancelled" })).status, "cancelled");
    await saveReport(path, buildReport({ ...env, CI_CHECK_SHA: "b".repeat(40) }));
    await assert.rejects(loadReportOrFailure(path, context), /provenance mismatch/);
