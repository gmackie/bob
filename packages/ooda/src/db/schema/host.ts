...
export const hostTurnExecutions = oodaSchema.table(
  ...
  (t) => ({
    ...
    preferredProvider: t.varchar({ length: 32 }),
    ...
    nativeTurnId: t.text(),
    runtimeTransport: t.varchar({ length: 32 }),
    ...
    completionIdempotencyKey: t.text(),
    completionFingerprint: t.text(),
  ...
  (t) => [
    ...
    uniqueIndex("host_turn_executions_completion_idempotency_uidx").on(
