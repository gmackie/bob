...
def create_app(overrides: dict[str, str] | None = None) -> FastAPI:
    ...
    # Starlette applies the most recently added middleware outermost. Keep
    # service authentication outside CORS so unauthenticated preflight requests
    # cannot bypass the fail-closed boundary on protected routes.
    app.add_middleware(
