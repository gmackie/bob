...
export abstract class BaseAgentAdapter implements AgentAdapter {
  ...
  async checkAvailability(): Promise<{
    ...
    try {
      ...
      return {
        isAvailable: false,
        statusMessage:
          error instanceof Error ? error.message : "Command not found",
      };
    }
  }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      ptyProcess.onData((data: string) => {
        ...
        if (!spawned && this.isAgentReady(data, output)) {
          spawned = true;
          console.log(
            `${this.name} PTY ready for worktree ${worktreePath} with PID ${ptyProcess.pid}`,
          );
          resolve(ptyProcess);
        }
      });
      ...
      ptyProcess.onExit(() => {
        ...
        if (!spawned) {
          reject(
            new Error(
              `${this.name} PTY process exited unexpectedly. Output: ${output}`,
            ),
          );
        }
      });
      ...
      const timeout = setTimeout(() => {
        if (!spawned) {
          ...
          reject(
            new Error(
              `${this.name} PTY failed to start within timeout. Output: ${output}`,
            ),
          );
        }
      }, 10000);
      ...
      setTimeout(() => {
        if (!spawned) {
          ...
          console.log(
            `${this.name} PTY assumed ready for worktree ${worktreePath}`,
          );
          resolve(ptyProcess);
        }
      }, 3000);
  ...
  abstract getSpawnArgs(options?: { interactive?: boolean; port?: number }): {
    command: string;
    args: string[];
  ...
  protected isAgentReady(data: string, fullOutput: string): boolean {
    ...
    return (
      fullOutput.toLowerCase().includes(this.name.toLowerCase()) ||
