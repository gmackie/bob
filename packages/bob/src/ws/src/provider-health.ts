/**
 * How a provider's health reads to a person, and what they should do about it.
 *
 * Lives in @bob/ws — beside the wire types it interprets — because the web
 * dashboard, the mobile node list and the tablet cockpit all render these same
 * lights. A copy per surface is how they drift into disagreeing about the same
 * agent, which this repo has already paid for: two settings routers, two
 * auth-table sets, and a latch three processes each read once and never again.
 *
 * The remedies mirror DispatchRemedy in bob-execution's dispatch-gate. Getting
 * them wrong is not cosmetic: offering "Sign in" for an exhausted balance is
 * how an operator loops on an action that cannot help.
 */
