...
import { workItems, taskRuns } from "./work-items.js";
...
export const forgeRevisions = pgTable(
  ...
  (t) => ({
    ...
    gates: t.json().$type<{ name: string; status: string; startedAt?: string; finishedAt?: string }[]>().default([]),
...
export const forgeRunEvents = pgTable(
  ...
  (t) => ({
    ...
    testStatus: t.text(),
    artifactRefs: t.json().$type<{ type: string; url?: string; description?: string }[]>().default([]),
    createdAt: t.timestamp({ mode: "string" }).defaultNow().notNull(),
  }),
  (table) => [
