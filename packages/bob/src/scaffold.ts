...
const TEMPLATE_REPO = "https://github.com/gmackorg/bob.git";
    ...
    if (
      ...
      try {
        ...
        if (content.includes("@bob/")) {
          content = content.replace(/@bob\//g, `${scope}/`);
  ...
  for (const app of ["nextjs", "expo", "tanstack-start"]) {
    ...
    if (fs.existsSync(appPkgPath)) {
      ...
      for (const pkgName of packagesToPrune) {
        delete pkg.dependencies?.[`@bob/${pkgName}`];
        delete pkg.devDependencies?.[`@bob/${pkgName}`];
      }
