...
export async function publicApiCreateArtifact(
  ...
  if (input.metadata?.kind === "trace_reference") {
    ...
    const workItem = run.workItemId && run.workspaceId
      ? await ctx.db.query.workItems.findFirst({
          where: and(eq(workItems.id, run.workItemId), eq(workItems.workspaceId, run.workspaceId)),
          columns: { id: true, externalId: true },
        })
      : undefined;
    if (run.workItemId && !workItem) {
      throw new TRPCError({ code: "BAD_REQUEST", message: "Run work item does not belong to its workspace" });
    }
      ...
      try {
        ...
        if (fg) {
          ...
          if (fgId) await fg.recordTrace(fgId, {
            ...
            outcome: run.status === "completed" ? "success" : run.status === "failed" ? "error" : "running", captureState: sampled ? "pending" : "sampled_out", services: ["ooda-runner"],
          });
        }
      } catch {
    ...
    if (taskRun?.planningProvider === "linear" && run.workspaceId && workItem?.externalId) {
      try {
        ...
        if (integration?.apiKey && integration.linearApiUrl) {
          ...
          await reportKanbangerTrace({ apiUrl: integration.linearApiUrl, apiKey: integration.apiKey }, workItem.externalId, {
            taskRunId: taskRun.id, attemptId: spanId, traceId, rootSpanId: spanId,
