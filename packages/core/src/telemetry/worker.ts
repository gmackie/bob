/** Dependency-free OTLP/HTTP JSON request spans for Cloudflare Workers. */
interface ExecutionContext {
  waitUntil(promise: Promise<unknown>): void;
}

type FetchHandler<E, C> = (request: Request, env: E, ctx: C) => Promise<Response>;
...
function traceParent(header: string | null) {
  ...
  return { traceId: match[1]!, parentSpanId: match[2]!, sampled: (parseInt(match[3]!, 16) & 1) === 1 };
}
...
function routeName(pathname: string) {
  ...
  return parts.map((part, i) => {
    ...
    const pageId = i === 2 && ["sessions", "threads", "work-items", "runs", "pull-requests", "projects", "nodes", "providers", "device"].includes(parts[1]!);
    const apiId = i === 4 && parts[1] === "api" && parts[2] === "v1" &&
      (["tts-streams", "conversations", "workspaces", "runs"].includes(parts[3]!) ||
        (parts[3] === "work-items" && parts[5] === "runs"));
    if (pageId || apiId || previous === "tts-streams" || previous === "conversations" || previous === "threads") return ":id";
...
function exportConfiguration(env: Record<string, unknown> | undefined, options: WorkerTelemetryOptions) {
  const specific = !options.endpoint && setting(env, "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT");
  const endpoint = new URL(specific || options.endpoint || setting(env, "OTEL_EXPORTER_OTLP_ENDPOINT") || setting(env, "OTEL_ENDPOINT") || "https://otlp.forgegraf.com");
  if (!["http:", "https:"].includes(endpoint.protocol) || endpoint.username || endpoint.password) throw new ExportFailure("invalid_configuration");
  if (!specific) endpoint.pathname = `${endpoint.pathname.replace(/\/+$/, "")}/v1/traces`;
  endpoint.hash = "";
  const headers = new Headers();
  const configuredHeaders = setting(env, "OTEL_EXPORTER_OTLP_TRACES_HEADERS") ?? setting(env, "OTEL_EXPORTER_OTLP_HEADERS");
  for (const pair of configuredHeaders?.split(",") ?? []) {
    const separator = pair.indexOf("=");
    if (separator < 1) throw new ExportFailure("invalid_configuration");
    headers.set(pair.slice(0, separator).trim(), decodeURIComponent(pair.slice(separator + 1).trim()));
  }
  headers.set("content-type", "application/json");
  const configuredTimeout = Number(setting(env, "OTEL_EXPORTER_OTLP_TRACES_TIMEOUT") ?? setting(env, "OTEL_EXPORTER_OTLP_TIMEOUT") ?? 5000);
  const timeout = Number.isFinite(configuredTimeout) && configuredTimeout > 0 ? Math.min(configuredTimeout, 10_000) : 5000;
...
async function readAcknowledgement(response: Response) {
  ...
  text += decoder.decode();
  let result;
  try { result = text ? JSON.parse(text) : {}; } catch { throw new ExportFailure("invalid_response"); }
  if (!result || typeof result !== "object" || Array.isArray(result)) throw new ExportFailure("invalid_response");
  ...
  if (partial && (Number(partial.rejectedSpans) > 0 || partial.errorMessage)) {
    ...
    diagnostic("partial_success", { rejectedSpans: Number.isFinite(rejected) ? rejected : 0 });
  }
}

async function pushSpan(env: Record<string, unknown> | undefined, options: WorkerTelemetryOptions, body: string) {
  let config;
  try { config = exportConfiguration(env, options); } catch { diagnostic("invalid_configuration"); return; }
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      if (response) {
        ...
        if (![429, 502, 503, 504].includes(response.status) || attempt === 2) return;
      }
      const retryAfter = response?.headers.get("retry-after");
      const requestedDelay = retryAfter ? (/^\d+$/.test(retryAfter) ? Number(retryAfter) * 1000 : Date.parse(retryAfter) - Date.now()) : 0;
      const delay = Math.max(100 + Math.random() * 100, Number.isFinite(requestedDelay) ? requestedDelay : 0);
      if (Date.now() + delay >= deadline) { diagnostic("retry_budget_exhausted"); return; }
      await new Promise((resolve) => setTimeout(resolve, delay));
    }
  } catch (error) {
    diagnostic(controller.signal.aborted ? "timeout" : error instanceof ExportFailure ? error.message : "invalid_response");
  } finally { clearTimeout(timer); }
}

/** Instruments the response headers boundary; never reads an application body. */
export function wrapFetch<E extends Record<string, unknown> | undefined, C extends ExecutionContext>(
  handler: FetchHandler<E, C>,
  options: WorkerTelemetryOptions,
): FetchHandler<E, C> {
  return async (request, env, ctx) => {
    if (setting(env, "OTEL_DISABLED")?.toLowerCase() === "true" || setting(env, "OTEL_SDK_DISABLED")?.toLowerCase() === "true") return handler(request, env, ctx);
    const parent = traceParent(request.headers.get("traceparent"));
    const sampled = parent ? parent.sampled : Math.random() < (options.sampleRate ?? 1);
    if (!sampled) return handler(request, env, ctx);
    ...
    try {
      ...
      const resourceAttributes = [
        ...
        { key: "deployment.environment", value: { stringValue: setting(env, "FG_STAGE") ?? setting(env, "NODE_ENV") ?? "production" } },
      ];
      const body = JSON.stringify({ resourceSpans: [{
        resource: { attributes: resourceAttributes },
        scopeSpans: [{ scope: { name: "@gmacko/core/telemetry/worker" }, spans: [{
          traceId, spanId, parentSpanId: parent?.parentSpanId,
          ...
          attributes, status: { code: status >= 500 ? 2 : 0 },
        }] }],
      }] });
      try { ctx.waitUntil(pushSpan(env, options, body)); } catch { diagnostic("wait_until_unavailable"); }
    }
  };
}
