...
function validatePayload(kind: string, payload: unknown) {
  if (kind === "agent_run") {
    exactKeys(
      payload,
      ["runtime", "status", "durationMs", "turnCount"],
      "agent_run payload",
    );
    enumValue(
      payload.runtime,
      ["codex", "claude", "cursor", "grok", "opencode", "smol-agent", "other"],
      "runtime",
    );
    enumValue(
      payload.status,
      ["success", "failure", "cancelled", "blocked"],
      "status",
    );
  ...
  if (kind === "tool_invocation") {
    exactKeys(
      payload,
      ["toolKind", "resourceIdDigest", "outcome", "durationMs"],
      "tool_invocation payload",
    );
    enumValue(
      payload.toolKind,
      ["skill", "mcp", "computer_use", "shell", "other"],
      "toolKind",
    );
    if (
      payload.resourceIdDigest !== null &&
      (typeof payload.resourceIdDigest !== "string" ||
        !DIGEST.test(payload.resourceIdDigest))
    ) {
  ...
  if (kind === "engineering_outcome") {
    exactKeys(
      payload,
      ["outcomeType", "result", "durationMs"],
      "engineering_outcome payload",
    );
    enumValue(
      payload.outcomeType,
      ["tests", "build", "deploy", "review", "promotion"],
      "outcomeType",
    );
...
function buildRecord(input: WorkflowEventInput): WorkflowRecord {
  ...
  enumValue(
    input.kind,
    ["agent_run", "tool_invocation", "engineering_outcome"],
    "kind",
  );
  ...
  return {
    ...
    sessionIdDigest:
      input.sessionId === null
        ? null
        : digest("skillfleet-session-v1", input.sessionId),
    projectIdDigest:
      input.projectId === null
        ? null
        : digest("skillfleet-project-v1", input.projectId),
...
async function appendRecord(journalPath: string, record: WorkflowRecord) {
  ...
  if (bytes.byteLength > MAX_RECORD_BYTES)
    throw new Error("workflow record exceeds limit");
  ...
  try {
    ...
    if (!metadata.isFile())
      throw new Error("journal path must be a regular file");
    await handle.chmod(0o600);
    const result = await handle.write(bytes, 0, bytes.byteLength, null);
    if (result.bytesWritten !== bytes.byteLength)
      throw new Error("workflow record append was incomplete");
    await handle.sync();
  } finally {
    await handle.close();
  }
